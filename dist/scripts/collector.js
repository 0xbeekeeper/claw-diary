...
import { getEventsDir, getTodayFileName, getDataDir, estimateCost, loadConfig } from './types.js';
...
function setCurrentSession(sessionId, startTime) {
...
function savePendingCalls(calls) {
    const dir = getDataDir();
    mkdirSync(dir, { recursive: true });
    ...
    switch (command) {
        case 'session-start': {
            ...
            writeEvent({
                ...
                sessionId,
                type: 'session_start',
                model: hookData.model || undefined,
            });
            break;
        }
        case 'session-stop': {
            ...
            writeEvent({
                ...
                tokenUsage: hookData.tokenUsage ? {
                    input: hookData.tokenUsage.input || 0,
                    output: hookData.tokenUsage.output || 0,
                    estimatedCost: estimateCost(hookData.model || 'default', hookData.tokenUsage.input || 0, hookData.tokenUsage.output || 0),
                } : undefined,
                model: hookData.model || undefined,
            });
            clearCurrentSession();
        ...
        case 'before': {
            ...
            if (config.recordingLevel === 'minimal')
                break;
            if (!session) {
        ...
        case 'after': {
            ...
            if (config.recordingLevel === 'minimal')
                break;
            const toolName = hookData.toolName || 'unknown';
            const resultData = hookData.result;
