 ...
 */
import { getDateFileName, loadEventsForDate, formatDuration, formatCost, formatTokens } from './types.js';
// ── Session grouping ──
...
function generateSessionDescription(toolCalls, toolResults, topTools, failureCount) {
    const parts = [];
...
function generateInsights(sessions, events, totalToolCalls, totalFailures) {
    const insights = [];
...
function generateDiaryMarkdown(dateStr, sessions, totalDuration, totalTokens, totalCost, totalToolCalls, insights) {
    const lines = [];
...
function categorizeSession(session) {
    ...
    if (tools.some(t => t.includes('edit') || t.includes('write'))) {
        if (tools.some(t => t.includes('bash')))
            return 'Coding & Testing';
        return 'Code Editing';
    }
    if (tools.some(t => t.includes('web') || t.includes('search')))
        return 'Research';
    if (tools.some(t => t.includes('read') || t.includes('grep') || t.includes('glob')))
        return 'Code Review';
    if (tools.some(t => t.includes('bash')))
