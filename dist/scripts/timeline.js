...
import { getDataDir, getDateFileName, loadEventsForDate, loadEventsForDays } from './types.js';
...
function getEventIcon(event) {
    ...
    if (tool.includes('read'))
        return '\uD83D\uDCD6';
    if (tool.includes('grep') || tool.includes('glob'))
        return '\uD83D\uDD0D';
    if (tool.includes('edit') || tool.includes('write'))
        return '\u270F\uFE0F';
    if (tool.includes('bash'))
...
function computeSummary(events) {
    ...
    for (const e of events) {
        ...
        if (e.type === 'tool_result' && e.result && !e.result.success)
            totalFailures++;
    }
    const sessions = [];
    for (const [sid, sevents] of sessionMap) {
        ...
        sevents.forEach(e => {
            if (e.tokenUsage) {
                stok += e.tokenUsage.input + e.tokenUsage.output;
                scost += e.tokenUsage.estimatedCost;
            }
        });
        const tc = new Map();
        calls.forEach(e => { if (e.toolName)
            tc.set(e.toolName, (tc.get(e.toolName) || 0) + 1); });
    ...
    if (totalToolCalls > 20)
        mood = 'productive';
    if (totalFailures > totalToolCalls * 0.3)
        mood = 'bumpy';
    if (totalToolCalls === 0)
        mood = 'quiet';
    if (sessions.length >= 3 && totalFailures === 0)
        mood = 'stellar';
...
function categorize(toolNames) {
    ...
    if (lower.some(t => t.includes('web') || t.includes('search')))
        return 'Research';
    if (lower.some(t => t.includes('edit') || t.includes('write'))) {
        if (lower.some(t => t.includes('bash')))
            return 'Coding & Testing';
        return 'Writing';
    }
    if (lower.some(t => t.includes('bash')))
        return 'DevOps';
    if (lower.some(t => t.includes('read') || t.includes('grep')))
        return 'Code Review';
    return 'Activity';
}
...
// ── HTML generation ──
export function generateTimelineHtml(events, title) {
    ...
    const moodEmoji = { quiet: '\uD83C\uDF19', calm: '\u2615', productive: '\uD83D\uDD25', bumpy: '\u26C8\uFE0F', stellar: '\u2728' };
    const moodWord = { quiet: 'quiet', calm: 'calm', productive: 'productive', bumpy: 'a bit bumpy', stellar: 'stellar' };
