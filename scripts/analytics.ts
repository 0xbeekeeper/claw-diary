...
import { DiaryEvent, DiaryAnalytics, getEventsDir, getDataDir, getDateFileName, loadEventsForDate, loadEventsForDays, formatDuration, formatCost } from './types.js';
import { generateTimelineHtml } from './timeline.js';
import { generateDailySummary } from './summarizer.js';

// ── Event loading ──

function loadAllEvents(): DiaryEvent[] {
...
export function generateStats(): DiaryAnalytics {
  // Load 30 days of data once into a cache
  const dayCache = new Map<string, DiaryEvent[]>();
  ...
  const weekEvents: DiaryEvent[] = [];
  const monthEvents: DiaryEvent[] = [];
  let dayIdx = 0;
  for (const [, events] of dayCache) {
    monthEvents.push(...events);
    if (dayIdx < 7) weekEvents.push(...events);
    dayIdx++;
  }
  ...
  const costByToolType: Record<string, number> = {};
  ...
  // Top tools
  const toolCountMap = new Map<string, { count: number; cost: number }>();
  for (const e of monthEvents) {
...
function sumCost(events: DiaryEvent[]): number {
...
): DiaryAnalytics['patterns'] {
  const patterns: DiaryAnalytics['patterns'] = [];
...
function searchEvents(query: string): DiaryEvent[] {
...
function exportData(format: string): void {
  ...
  switch (format) {
    ...
    default: {
      ...
      const files = readdirSync(dir).filter(f => f.endsWith('.jsonl')).sort();
      const sections: string[] = ['# Claw Diary Export', ''];
      for (const file of files) {
...
function clearData(confirmed: boolean): void {
  if (!confirmed) {
